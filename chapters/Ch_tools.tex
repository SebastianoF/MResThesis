...
This second kind of exponential is called affine exponential\footnote{
	Little note about the notation: to distinguish the affine $\exp$ and $\log$ from the Lie $\exp$ and $\log$ presented in the previous section, the affine will always have the subscript of the point of application even when it is the identity.
	}
...
\begin{theorem}\label{th:local_approximation_theorem}
	...
	\end{align*}
	\begin{align*}
	 \mathbf{u}_{e}^{\parallel} := D(L_{b^{-1}})_{e}( -\Pi(\alpha)_{a}^{b}(\mathbf{u}))
	\end{align*}
	...
	\begin{align*}
		\euclideanMetric{
			\log(\exp_{e}(\mathbf{u}_{e}^{\parallel} ))
			-
			\log(\exp_{e}\big(\frac{\mathbf{v}}{2}\big)   
...
\end{align}
\noindent
Further theoretical developments are beyond the aim of this research, but the reader can refer to the bibliography, or to the last version of \emph{some notes on differential geometry for diffeomorphic image registration} on github \cite{xxx}, where proofs and examples are provided, and relations between equations \ref{eq:DL_DR} and parallel transport are investigated.
...
\end{align*}
These numerical approximations of the log-composition can be considered as a first step but are not satisfactory since they do not provide any condition to consider the error carried by each term.
...
As shown in the appendix of \cite{klarsfeld1989baker} the terms of the BCH can be 

The second method for the computation of the Log-composition relies on the Taylor expansion.\\
...
\end{align*}
In the appendix of \cite{klarsfeld1989baker} adjoint action are used to provide an expansion of the BCH formula. This can be rewritten as
\begin{align*}
