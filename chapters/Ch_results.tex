...
Results are consistent with the one obtained for synthetic data except when the norm of $\mathbf{u}_{A}^{B}$ si below or equal $0.0045$. In this case parallel transport method provides better results than the $\text{BCH}^1$.
We can see in particular that the difference between $\text{BCH}^0$ and $\text{BCH}^{1}$ can be seen only in some cases after the third decimal. Results obtained with $\text{BCH}^{1.5}$ and $\text{BCH}^2$ are not showed because they do not lead to any improvement, in particular for small SVF. 
