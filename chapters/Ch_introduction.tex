...
\noindent
Far from being a complete overview of all of the possible frameworks, it does not take into account the fact that each version or implementation inevitably involves different needs and consequent challenges. For accurate surveys in medical image registration see for example \cite{Sotiras:survey:13}, \cite{zitova2003image} . \\
In the following subsections we will going from the generalized framework to some specific algorithms. We are interested in particular in the parametrization of the \emph{diffeomorphisms} of the latest important algorithms: the LDDMM and the diffeomorphic Demons.
...
% discretization 
In the LDDMM implementation, the data structure utilized to store deformation fields are 5-dimensional matrices
...
It has been presented several approaches to face the problem of the computation of the update: diffeomorphic demons compute each new diffeomorphisms as the composition between the diffeomorphism obtained at the previous step $\varphi_{k}$ with the exponential of the SVF $\delta V_{k}$ obtained by the optimization algorithm :
\begin{align*}
